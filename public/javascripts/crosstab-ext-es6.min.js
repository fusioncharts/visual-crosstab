...
/***/ function(module, exports) {
	...
	// max = -Infinity,
	// min = Infinity,
	p={},q={},s={},t={},u=this.globalData[this.dimensions[this.dimensions.length-1]];if(h.push.apply(h),f=h.filter(v=>{return''!==v}),g=f.join('|'),o=this.hash[this.matchHash(g,this.hash)],o){for(let v=0,w=o.length;v<w;v++)n=this.mc.createDataProcessor(),n.filter(o[v]),m.push(n);return p=this.dataStore.getData(m),p=p[p.length-1],q={config:{dimension:[this.dimensions[this.dimensions.length-1]],measure:[c],seriesType:'SS',aggregateMode:this.aggregation,categories:u,config:this.chartConfig},datastore:p},s=this.mc.dataAdapter(q),t=s.getLimit(),[{max:t.max,min:t.min},{type:this.chartType,width:'100%',height:'100%',configuration:s}]}}dragListener(b){function c(o){let w,z,p=0,q=m[o],s=q.redZone,t=q.origLeft,u=q.index,v={};for(p=o;p--;)w=m[p],z=m[p+1],parseInt(q.graphics.style.left)<w.redZone&&(s=w.redZone,t=w.origLeft,u=w.index,z.adjust+=parseInt(w.graphics.style.width),w.origLeft=z.origLeft,w.redZone=z.redZone,w.index=z.index,w.graphics.style.left=w.origLeft+'px',v=m[p+1],m[p+1]=m[p],m[p]=v);// Setting new values for dragitem
	q.origLeft=t,q.redZone=s,q.index=u}// Getting only labels
	let m,f=this.storeParams.config,g=f.dimensions||[],h=0,n=this;// let end
	b=b[1],g=g.slice(0,g.length-1),h=g.length,m=b.slice(0,h);for(let o=0;o<h;++o){let p=m[o].graphics,q=m[o];q.cellValue=g[o],q.origLeft=parseInt(p.style.left),q.redZone=q.origLeft+parseInt(p.style.width)/2,q.index=o,q.adjust=0,q.origZ=p.style.zIndex,this._setupDrag(q.graphics,function(t){p.style.left=q.origLeft+t+q.adjust+'px',p.style.zIndex=1e3,c(q.index)},function(){let t=!1,u=0;for(q.adjust=0,p.style.zIndex=q.origZ,p.style.left=q.origLeft+'px';u<h;++u)n.dimensions[o]!==m[o].cellValue&&(n.dimensions[o]=m[o].cellValue,t=!0);t&&(n.globalData=n.buildGlobalData(),n.renderCrosstab())})}}_setupDrag(b,c,f){function g(n){c(n.clientX-h,n.clientY-m)}let h=0,m=0;b.addEventListener('mousedown',function(n){h=n.clientX,m=n.clientY,b.style.opacity=0.8,window.document.addEventListener('mousemove',g)}),window.document.addEventListener('mouseup',function(){b.style.opacity=1,window.document.removeEventListener('mousemove',g),f()})}filterGen(b,c){return f=>f[b]===c}}module.exports=CrosstabExt;
